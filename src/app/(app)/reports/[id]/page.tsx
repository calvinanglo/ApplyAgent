  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">{report.company}</h1>
          <p className="text-lg text-muted-foreground">{report.role}</p>
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          {report.score && (
            <div className={`${scoreColor} text-white rounded-lg px-4 py-2 text-center`}>
              <p className="text-2xl font-bold leading-none">{report.score}</p>
          ...
          <div className="flex gap-2">
            ...
            <Link href={`/resume?report_id=${id}`}>
              <Button variant="outline" size="sm"><FileDown className="size-4 mr-1" />Resume PDF</Button>
            </Link>
          </div>
        </div>
      </div>
