-- Allow free evaluations to be used for any credit-costing action, not just evaluations
...
begin
  ...
  -- Check free tier (3 free uses total, works for any action)
  if v_free_used < 3 then
    ...
    values (p_user_id, 0, v_balance, 'free_tier', p_action, p_reference_id, coalesce(p_description, 'Free tier usage'));
